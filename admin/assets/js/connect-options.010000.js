...
if( !window.bizpanda.connectOptions ) {
...
(function($) {
	...
	window.bizpanda.connectOptions = {
		...
		getConnectOptions: function() {
			...
			var options = {
				...
				connectButtons: {
					...
					vk: {
						...
						actions: gerOrder('#opanda_vk_actions')
					}
				},
			...
			if( window.bizpanda.connectOptions.filterOptions ) {
				options = window.bizpanda.connectOptions.filterOptions(options);
			}
		...
		hideEmptyDisabledActions: function() {
			...
			$(".opanda-actions-disabled").each(function() {
				if( $(this).find(".opanda-action").length > 0 ) {
					return;
				}
				$(this).hide();
			});
		...
		toogleButton: function(name) {
			...
			if( $button.hasClass('opanda-on') ) {
				this.deactivateButton(name);
			} else {
		...
		activateButtonAction: function(buttonName, actionName, setup) {
			...
			if( $action.is('.opanda-action-disabled') ) {
				return;
			}
			...
			if( !setup ) {
				this.updateActionsResult(buttonName, actionName);
			}
		},
		...
		toogleOptions: function(common, button, action) {
			...
			if( !$options.is(":visible") ) {
				this.showOptions(common, button, action);
			} else {
				this.hideOptions(common, button, action);
			}
		},
