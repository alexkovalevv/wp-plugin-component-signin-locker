<?php
	...
	add_filter('bizpanda_signin-locker_type_metaboxes', 'opanda_signin_locker_metaboxes', 10, 1);
	...
	function opanda_signin_locker_activation($plugin, $helper)
	{
		...
		$helper->addPost('opanda_default_signin_locker_id', array(
			...
			'opanda_facebook_actions' => BizPanda::hasPlugin('optinpanda')
				? 'signup'
				: 'signup',
			'opanda_twitter_actions' => BizPanda::hasPlugin('optinpanda')
				? 'signup'
				: 'signup',
			'opanda_google_actions' => BizPanda::hasPlugin('optinpanda')
				? 'signup'
				: 'signup',
			'opanda_linkedin_actions' => BizPanda::hasPlugin('optinpanda')
				? 'signup'
				: 'signup',
			'opanda_email_actions' => BizPanda::hasPlugin('optinpanda')
				? 'signup'
				: 'signup',
			'opanda_mobile' => 1,
			'opanda_highlight' => 1,
	...
	add_action('bizpanda_after_activation', 'opanda_signin_locker_activation', 10, 2);
	...
	function opanda_register_signin_locker_themes($item = 'signin-locker')
	{
		...
		if( !onp_build('free') ) {
			...
			OPanda_ThemeManager::registerTheme(array(
				...
				'previewHeight' => 400,
				'hint' => sprintf(__('This theme is available only in the <a href="%s" target="_blank">premium version</a> of the plugin', 'bizpanda-signin-locker'), opanda_get_premium_url(null, 'themes')),
				'items' => array('step-to-step', 'signin-locker', 'email-locker')
			));
	...
	add_action('bizpanda_register_themes', 'opanda_register_signin_locker_themes');
	 ...
	 * @since 1.0.0
	 */
	function bizpanda_help_page_what_is_signin_locker($manager)
	{
		require BIZPANDA_SIGNIN_LOCKER_DIR . '/admin/help/what-is-it.php';
	}

	add_action('bizpanda_help_page_what-is-signin-locker', 'bizpanda_help_page_what_is_signin_locker');
	 ...
	 * @since 1.0.0
	 */
	function bizpanda_help_page_usage_example_signin_locker($manager)
	{
		require BIZPANDA_SIGNIN_LOCKER_DIR . '/admin/help/usage-example.php';
	}

	add_action('bizpanda_help_page_usage-example-signin-locker', 'bizpanda_help_page_usage_example_signin_locker');
	...
	function opanda_signin_locker_stats_screens($screens)
	{
		...
		$screens = array(
			...
			'summary' => array(
				'title' => __('<i class="fa fa-search"></i> Summary', 'bizpanda-signin-locker'),
				'description' => __('The page shows the total number of unlocks for the current locker.', 'bizpanda-signin-locker'),
			...
			'profits' => array(
				'title' => __('<i class="fa fa-usd"></i> Benefits', 'bizpanda-signin-locker'),
				'description' => __('The page shows benefits the locker brought for your website.', 'bizpanda-signin-locker'),
			...
			'channels' => array(
				'title' => __('<i class="fa fa-search-plus"></i> Channels', 'bizpanda-signin-locker'),
				'description' => __('The page shows which ways visitors used to unlock the content.', 'bizpanda-signin-locker'),
			...
			'bounces' => array(
				'title' => __('<i class="fa fa-sign-out"></i> Bounces', 'bizpanda-signin-locker'),
				'description' => __('The page shows major weaknesses of the locker which lead to bounces. Hover your mouse pointer on [?] in the table, to know more about a particular metric.', 'bizpanda-signin-locker'),
		...
		if( !onp_build('free') ) {
			...
			$screens['skips'] = array(
				'title' => __('<i class="fa fa-tint"></i> Skips', 'bizpanda-signin-locker'),
				'description' => __('The chart shows how many users skipped the locker by using the Timer or Close Icon, comparing to the users who unlocked the content.', 'bizpanda-signin-locker'),
	...
	add_filter('bizpanda_signin-locker_stats_screens', 'opanda_signin_locker_stats_screens', 10, 1);
