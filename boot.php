<?php
	...
	load_textdomain('bizpanda-signin-locker', BIZPANDA_SIGNIN_LOCKER_DIR . '/langs/bizpanda-signin-locker-' . get_locale() . '.mo');
	...
	if( !function_exists('opanda_register_signin_locker') ) {
		...
		function opanda_register_signin_locker($items)
		{
			...
			if( onp_build('free') ) {
				...
				$items['signin-locker'] = array(
					'name' => 'signin-locker',
					'type' => 'free',
					'title' => __('Sign-In Locker', 'bizpanda-signin-locker'),
					'help' => opanda_get_help_url('what-is-signin-locker'),
					'description' => __('<p>Locks the content until the user signs in through social networks.</p><p>You can set up various actions to be performed to sign in (e.g. subscribe, follow, share).</p>', 'bizpanda-signin-locker'),
				...
				$items['signin-locker'] = array(
					...
					'title' => __('Sign-In Locker', 'bizpanda-signin-locker'),
					'help' => opanda_get_help_url('what-is-signin-locker'),
					'description' => __('<p>Locks the content until the user signs in through social networks.</p><p>You can set up various social actions to be performed to sign in (e.g. subscribe, follow, share).</p>', 'bizpanda-signin-locker'),
					'shortcode' => 'signinlocker',
					'plugin' => $plugin
				);
			}
		...
		add_filter('bizpanda_items', 'opanda_register_signin_locker', 2);
		...
		function opanda_signin_locker_options($options, $id)
		{
			...
			if( in_array('vk', $actions) ) {
				$options['connectButtons']['vk'] = array(
					'proxy' => opanda_proxy_url('opanda_vk_connect'),
					'actions' => opanda_signin_locker_get_actions($id, 'vk_actions'),
			...
			if( $hasEmailForm ) {
				...
				$options['subscription']['form'] = array(
					...
					'type' => opanda_get_item_option($id, 'subscribe_name')
						? 'name-email-form'
						: 'email-form'
				);
			}
		...
		add_filter('bizpanda_signin-locker_item_options', 'opanda_signin_locker_options', 10, 2);
		...
		add_action('bizpanda_request_assets_for_signin-locker', 'opanda_signin_locker_assets', 10, 4);
		...
		FactoryShortcodes000::register('OPanda_SignInLockerShortcode', $bizpanda);
	}